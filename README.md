...
```
tanzu apps workload create node-express-demo -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/node-express-demo-source \
  --type web
```
...
```
tanzu apps workload get node-express-demo
```
